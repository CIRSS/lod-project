...
<xsl:stylesheet 
  ...
  <!-- Global Variables: these spreadsheet columns contain local identifiers of related resources; 
       ids can be prefixed to yield resource URIs  -->
  ...
  <xsl:template match='/'>
    <xsl:for-each select="//s:Row[position()>1][s:Cell/s:NamedCell/@s:Name='localId']">

       <!-- this spreadsheet column contains local identifier of the resource being described by the row,
            from which the resource URI and file location for saving can be derived -->
       <xsl:variable name="localId" select="./s:Cell[s:NamedCell/@s:Name='localId']/s:Data"/>
       ...
       <xsl:result-document href="{$saveAs}" method='text' exclude-result-prefixes="#all" 
         ...
         <xsl:variable name="myLinks">
           <xsl:for-each select="./s:Cell[s:NamedCell/@s:Name='VIAF_Link']/s:Data">    
             <value>"http://viaf.org/viaf/<xsl:value-of select="."/>"</value>
           </xsl:for-each>
           <xsl:for-each select="./s:Cell[s:NamedCell/@s:Name='EN_WIKIPEDIA' or s:NamedCell/@s:Name='FR_WIKIPEDIA']/s:Data">    
         ...
         <xsl:call-template name="parseValues">
           <xsl:with-param name="valueKey">honorificPrefix</xsl:with-param></xsl:call-template>  
         <xsl:call-template name="parseValues">
         ...
         <xsl:call-template name="parseValues">
           <xsl:with-param name="valueKey">jobTitle</xsl:with-param></xsl:call-template>             
         <xsl:call-template name="parseValues">
           <xsl:with-param name="valueKey">nationality</xsl:with-param></xsl:call-template>                        
         <xsl:call-template name="parseValues">
           <xsl:with-param name="valueKey">memberOf</xsl:with-param></xsl:call-template>
         <xsl:call-template name="parseValues">
         ...
         <xsl:call-template name="parseValues">
           <xsl:with-param name="valueKey">spouse</xsl:with-param></xsl:call-template>
         <xsl:call-template name="parseValues">
        ...
        <xsl:for-each select="tokenize(./s:Cell[s:NamedCell/@s:Name=$valueKey]/s:Data, '; ')">
          <xsl:choose>
            <xsl:when test="contains($needPrefix, $valueKey)">
              <value>"<xsl:value-of select="$prefix"/><xsl:value-of select="replace(., '&quot;', '\\&quot;')"/>"</value>
            </xsl:when>
            <xsl:otherwise>
              <value>"<xsl:value-of select="replace(., '&quot;', '\\&quot;')"/>"</value>                    
            </xsl:otherwise>
          </xsl:choose>
        </xsl:for-each>
      </xsl:variable>
